  ...
  try {
    ...
    const { data: result, source } = await cachedFetchJsonWithMeta<{ summary: string; model: string; tokens: number }>(
      ...
      async () => {
        ...
        const isAnthropic = extraBody?.isAnthropic === true;
        const bodyContent = isAnthropic ? {
          model,
          system: systemPrompt,
        ...
        const tokens = isAnthropic ? ((data.usage?.input_tokens || 0) + (data.usage?.output_tokens || 0)) : ((data.usage?.total_tokens as number) || 0);
        let rawContent = '';
        if (isAnthropic) {
          rawContent = typeof data.content?.[0]?.text === 'string' ? data.content[0].text.trim() : '';
        } else {
